...
//---------------------------------------------------------------------------
UserInterface* UserInterface::p_instance_ = nullptr;
//---------------------------------------------------------------------------
...
void UserInterface::showValidateAccountOnScreen(int validCode, const string& accNum) const {
    ...
    {
		case OPEN_ACCOUNT:
			cout << "\nTHE ACCOUNT (NUMBER: " << accNum << ") IS ALREADY OPEN!";
			break;
    	case VALID_ACCOUNT:		//account valid: it exists, is accessible with that card (and not already open)
...
Date UserInterface::readInValidDate(const Date& cd) const{
	...
	while (!isValid){
		aDate = readInDate();
		isValid = aDate.isValid(cd);
	}
	return aDate;
}
bool UserInterface::readInConfirmDeletion() const{
	char input;
	cout << "\nCONFIRM DELETION OF ABOVE TRANSACTIONS (Y/N): ";
	cin >> input;
...
int	UserInterface::readInSearchCommand() const{
	return (readInCommand());//TODO: check if invalid
}
...
UserInterface* UserInterface::getInstance(){
	return p_instance_ ? p_instance_ : (p_instance_ = new UserInterface);
}
